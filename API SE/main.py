...
def get_progressions(
    ...
    while current <= end:
        ...
        try:
            ...
            prog_cusps, prog_ascmc = swe.houses_armc(prog_ramc, data.latitude, eps_p, b"P")
            ...
            # ── 12 cuspides Placidus progressées (Rushman R.5.1 : progresser
            #    TOUTES les maisons, pas seulement Asc/MC). Additif : ne modifie
            #    aucun champ existant. cusps[i] = cuspide maison i+1. ──
            for i in range(12):
                cdeg = prog_cusps[i] % 360
                cuspides.append({
                    "house":             i + 1,
                    "longitude_absolue": round(cdeg, 4),
                    "signe":             ZODIAC_SIGNS_LIST[int(cdeg / 30) % 12],
                    "degre_dans_signe":  round(cdeg % 30, 4),
                })
        except Exception:
            pass
...
_DP_ANGLE_SHORT = {"Ascendant": "ASC", "Descendant": "DSC", "Milieu du Ciel": "MC", "Imum Coeli": "IC"}
...
def _dp_mundane_result(jd_birth, body, age, eps_deg):
    # Directions primaires MUNDANE Placidus semi-arc (all-to-all).
    # Valide contre Morinus (mode M, Placidus semiarc) a moins de 0.01 deg.
    lat_geo = body.birth.latitude
    lon_geo = body.birth.longitude
    dp_orb_yr = body.dp_orb_yr
    aspects_filter = body.aspects if body.aspects else list(_DP_ASPECTS.keys())
    eps_rad = eps_deg * _math.pi / 180.0

    cusps, ascmc = swe.houses(jd_birth, lat_geo, lon_geo, b"P")
    armc0 = ascmc[2]

    pts = {}
    natal_positions = {}
    for name, bid in _DP_SWE_BODY.items():
        try:
            xx, _f = swe.calc_ut(jd_birth, bid, swe.FLG_SWIEPH | swe.FLG_SPEED)
            plon = xx[0]
            plat = xx[1]
            if "Sud" in name:
                plon = (plon + 180.0) % 360.0
                plat = -plat
            pts[name] = (plon, plat)
            ra, dec = _dp_eq(plon, plat, eps_rad)
            natal_positions[name] = {"lon": round(plon, 6), "lat_ecl": round(plat, 6),
                                     "ra": round(ra, 6), "dec": round(dec, 6)}
        except Exception:
            continue
    pts["Ascendant"] = (ascmc[0] % 360.0, 0.0)
    pts["Descendant"] = ((ascmc[0] + 180.0) % 360.0, 0.0)
    pts["Milieu du Ciel"] = (ascmc[1] % 360.0, 0.0)
    pts["Imum Coeli"] = ((ascmc[1] + 180.0) % 360.0, 0.0)

    def _hp(armc, lonlat):
        try:
            r = swe.house_pos(armc % 360.0, lat_geo, eps_deg, lonlat, b"P")
        except Exception:
            return None
        return r[0] if isinstance(r, (tuple, list)) else r

    def _wrapdiff(h, t):
        return ((h - t + 6.0) % 12.0) - 6.0

    hp_nat = {}
    for n in pts:
        hp_nat[n] = _hp(armc0, pts[n])

    NAIBOD = _DP_NAIBOD
    a_lo = max(0.0, age - dp_orb_yr) * NAIBOD
    a_hi = (age + dp_orb_yr) * NAIBOD
    stepD = 0.02
    win_direct = []
    d = a_lo
    while d <= a_hi:
    ...
    for n in pts:
        prom_samples[n] = {
            "directe": [(x, _hp(armc0 + x, pts[n])) for x in win_direct],
            "converse": [(x, _hp(armc0 + x, pts[n])) for x in win_conv],
        }
    ...
    for prom in names:
        for sig in names:
            ...
            if hps is None:
                continue
            for aspN in aspects_filter:
                adeg = _DP_ASPECTS.get(aspN)
                if adeg is None:
                    continue
                off = int(round(adeg / 30.0))
                levels = [0] if off == 0 else [off, -off]
                for tag in ("directe", "converse"):
                    samples = prom_samples[prom][tag]
                    for lv in levels:
                        tgt = hps + lv
                        prev = None
                        for (xx2, hh) in samples:
                            g = None if hh is None else _wrapdiff(hh, tgt)
                            if prev is not None and prev[1] is not None and g is not None:
                                pd, pg = prev
                                if pg == 0.0 or (g * pg < 0 and abs(g - pg) < 6):
                                    a = pd
                                    b = xx2
                                    ga = pg
                                    for _i in range(30):
                                        m = (a + b) / 2.0
                                        gm = _wrapdiff(_hp(armc0 + m, pts[prom]), tgt)
                                        if ga * gm <= 0:
                                            b = m
                                        else:
                                            a = m
                                    ...
                                    if orbY <= dp_orb_yr:
                                        raw.append({
                                            "promissor": prom,
                                            "significator": _DP_ANGLE_SHORT.get(sig, sig),
                                            "aspect": aspN,
                                            "orbYears": round(orbY, 4),
                                            "orbDeg": round(orbY * NAIBOD, 4),
                                            "direction": tag,
                                            "exactAge": round(exact, 4),
                                        })
                            prev = (xx2, g)

    best = {}
    for h in raw:
        k = (h["promissor"], h["significator"], h["aspect"], h["direction"])
        if k not in best or h["orbYears"] < best[k]["orbYears"]:
            best[k] = h
    hits = sorted(best.values(), key=lambda z: z["orbYears"])
...
def calc_primary_directions(body: PrimaryDirectionsBody):
    ...
    if getattr(body, "frame", "zodiacal") == "mundane":
        return _dp_mundane_result(jd_birth, body, age, eps_deg)
