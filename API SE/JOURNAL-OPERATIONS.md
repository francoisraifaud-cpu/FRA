...
- **F2 (reboot noyau) — FAIT 2026-07-18** : `shutdown -r now` (fenêtre : site derrière portail). Retour : `uname -r` = **`6.8.0-136-generic`**, `reboot-required` absent, services `astro-api`/`nginx`/`docker`/`fail2ban` actifs, gotenberg up, `:8000`/`:3000` loopback, `https://api.spikka.eu/health` = 200, durcissement F1 persisté.
- **F6 (UFW) — FAIT 2026-07-18** : suppression des 4 règles redondantes `8000/3000` (n8n + dev) — rien n'écoute en public sur ces ports. Restent `22`/`80`/`443` + `443` restreint n8n/dev.
- **F3 (sauvegarde serveur) — FAIT 2026-07-18** : via API Hetzner Cloud (`astro-server` id `123032910`). **Backups automatiques activés** (`enable_backup` = success, fenêtre `02-06` UTC, rétention 7 quotidiens) + **snapshot manuel baseline** `astro-server DR baseline 2026-07-18` (image id `409950172`, 4,68 Go, `available`). RTO astro-server ramené à ~minutes via *rebuild from image*. Token Hetzner utilisé puis **révoqué** côté console (aucun token versionné).
- **F4 (monitoring uptime) — FAIT 2026-07-18** : sonde côté SITE (urbanisation : tout dans Sentry). Nouveau cron Vercel `/api/cron/health-probe` (5 min) qui pinge `http://api.spikka.eu/health` (moteur, **port 80**) + `${site}/api/health?deep=1` (site + PostgreSQL) → alerte Sentry `area:uptime` si KO. Portail exempte `/api/health` et `/api/cron` → pas de faux positif. Déployé preprod + prod. Fichiers SITE : `app/api/cron/health-probe/route.ts`, `lib/sentry-uptime-alerts.ts`, `vercel.json`.
- **Correctif sonde (même jour)** : 1ère version ciblait `https://api.spikka.eu/health` (443) → **faux positif Sentry** (timeout à 18:00) car le **443 est pare-feu-restreint** à n8n (`51.116.119.68`) + dev (`83.202.99.64`), inatteignable depuis l'egress Vercel. Basculé sur le **port 80 `/health`** (seul endpoint moteur public, non sensible). Bonne nouvelle : l'alerte reçue **valide le pipeline Sentry de bout en bout**.
