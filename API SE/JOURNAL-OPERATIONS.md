...
- **F4 (monitoring uptime) — FAIT 2026-07-18** : sonde côté SITE (urbanisation : tout dans Sentry). Nouveau cron Vercel `/api/cron/health-probe` (5 min) qui pinge `https://api.spikka.eu/health` (moteur) + `${site}/api/health?deep=1` (site + PostgreSQL) → alerte Sentry `area:uptime` si KO. Portail exempte `/api/health` et `/api/cron` → pas de faux positif. Déployé preprod + prod. Fichiers SITE : `app/api/cron/health-probe/route.ts`, `lib/sentry-uptime-alerts.ts`, `vercel.json`.
