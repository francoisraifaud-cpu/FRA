...
- **F3 (sauvegarde serveur) — FAIT 2026-07-18** : via API Hetzner Cloud (`astro-server` id `123032910`). **Backups automatiques activés** (`enable_backup` = success, fenêtre `02-06` UTC, rétention 7 quotidiens) + **snapshot manuel baseline** `astro-server DR baseline 2026-07-18` (image id `409950172`, 4,68 Go, `available`). RTO astro-server ramené à ~minutes via *rebuild from image*. ⚠️ Token Hetzner transmis en clair dans le chat → **à révoquer/régénérer** côté console (aucun token versionné).
