...
// --- MDSE v17 — A4/A5/A6 : helper relax conditionnelle pour codes engagement (MARIAGE/ENFANT) ---
...
const _A4_RELAX_CODES = new Set(["MARIAGE", "ENFANT"]);
const _A4_MIN_RAW_RATIO = 0.55;
const _A4_PROF_HOUSES = { MARIAGE: [5, 7], ENFANT: [4, 5] };
const _a4ShouldSkipMaleficPenalty = (sig) => {
  if (!_A4_RELAX_CODES.has(sig.code)) return null;
  if ((sig._rawRatio || 0) < _A4_MIN_RAW_RATIO) return null;
  const benM = (sig.matchedConditions || []).find(m => (m.strength || 0) >= 0.7 && /Jupiter|Vénus/i.test(m.detail || ""));
...
// Les codes "engagement" (MARIAGE, ENFANT) sont exemptés de la cascade de pénalités
// d'asymétrie conceptuelle (P1 natal promise, P56 maleficPenalty, CF1 counter penalty,
// P1' MDSE v2 transit dignity, P0/P0b T/S ratio, Q3 asymétrique, P6 RS Cluster).
// La tension (Saturne/Uranus) crée l'événement mais le bénéfique (Jupiter/Vénus) ou
// le Time Lord (Profection) le SIGNE en tant que contrat. Sans cette signature
// (= pas de garde-fou A4), un VERROU CONTRACTUEL en fin de cascade cap la conf à 45.
const _ENGAGEMENT_CODES = new Set(["MARIAGE", "ENFANT"]);
...
if (finalOutput.length > 0) {
    ...
    // Pour MARIAGE/ENFANT, après skip de toutes les pénalités d'asymétrie conceptuelle
    // (P1, P56, CF1, P1', P0, Q3, P6), si AUCUN garde-fou bénéfique n'est actif
    // (Jupiter/Vénus matched à str≥0.7  OU  profection annuelle M5/M7|M4/M5),
    // la confidence est cappée à 45 — la tension seule reste une CRISE, pas un MARIAGE.
    // Validation astrologue : retour n°7 Q2 (2026-05-04 06:40).
    try {
    _mdseResults.forEach(sig => {
      ...
      if (guard) {
        ...
        sig._engagementContractCap = { before: sig.confidence, after: 45, reason: "Pas de garde-fou bénéfique (ni Jupiter/Vénus matched, ni profection M5/7|4/5)" };
        sig.confidence = 45;
        sig.level = sig.confidence >= 55 ? "high" : sig.confidence >= 35 ? "medium" : "low";
      }
    });
