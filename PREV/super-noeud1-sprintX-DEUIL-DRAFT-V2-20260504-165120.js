...
if (finalOutput.length > 0) {
    ...
    try {
      ...
      const _subxEclipseOnPoint = (targetNames, orbMax) => {
        if (!Array.isArray(eclipsesData) || eclipsesData.length === 0) return null;
        for (const e of eclipsesData) {
          const dayMatch = (typeof daysArray !== "undefined") ? daysArray.find(d => d.date === e.date) : null;
          if (!dayMatch?.planetes) continue;
          const refDeg = e.astre === "Soleil" ? dayMatch.planetes.Soleil?.fullDegree : dayMatch.planetes.Lune?.fullDegree;
          if (refDeg == null) continue;
          for (const tgt of targetNames) {
            ...
            if (d <= orbMax) return { date: e.date, astre: e.astre, target: tgt, orb: Math.round(d * 10) / 10 };
          }
        }
        return null;
      };
      ...
      _mdseResults.forEach(sig => {
        ...
        // BOOST FORCÉ : si critical_trauma OU sudden_end → conf >= 88
        // Q2-A (validé astrologue retour n°13) : pas de boost sur Hyleg seul (cas Mitterrand 1981).
        if ((criticalTrauma || suddenEnd) && sig.confidence < 88) {
    ...
    } catch(e) { console.log("[WARN] Sprint X DEUIL compounds:", e.message); }
