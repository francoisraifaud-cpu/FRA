...
if (finalOutput.length > 0) {
    ...
    // --- Sprint 3.1 (2026-05-04, validé astrologue retour n°10) : FILTRE CONCURRENCE ---
    // Si DEUIL/ACCIDENT/SANTE_DOWN ≥ 80 sur fenêtre overlap avec CARRIERE_UP,
    // CARRIERE_UP est cappé à 60 ("la survie prime sur le statut social").
    // Cible : 8 FP cohorte type "decès avec Jupiter→MC concomitant" (lennon, marie-antoinette, etc.)
    // → reconnaissance posthume légitime mais ne doit pas surclasser le décès lui-même.
    try {
      const _CARRIERE_COMPETITOR_CODES = new Set(["DEUIL", "ACCIDENT", "SANTE_DOWN"]);
      const _CARRIERE_COMPETITOR_THRESHOLD = 80;
      ...
      _mdseResults.forEach(sig => {
        ...
        if (conflict) {
          sig._carriereDemoted = {
            ...
            reason: `Filtre concurrence : ${conflict.code} conf=${conflict.confidence} ≥ ${_CARRIERE_COMPETITOR_THRESHOLD} overlap fenêtre`
          };
          sig.confidence = _CARRIERE_DEMOTION_CAP;
          sig.level = sig.confidence >= 55 ? "high" : sig.confidence >= 35 ? "medium" : "low";
        }
      });
    } catch(e) { console.log("[WARN] Sprint 3.1 filtre concurrence CARRIERE_UP:", e.message); }
