...
const _natalPromises = (() => {
  ...
  try {
    ...
    // [1] FRAGILITE_INNEE_SANTE
    _npAddTag("FRAGILITE_INNEE_SANTE", (() => {
      for (const v of ["Soleil","Ascendant"]) {
        if (natalDict[v]?.house === 6) {
          for (const a of ["Saturne","Neptune"]) {
            if (_npHasNatalAsp(v, a, _NP_HARD_ASPECTS, 2)) return true;
          }
        }
      }
      return false;
    ...
    // [2] TEMPETE_INTERIEURE
    _npAddTag("TEMPETE_INTERIEURE", natalConfigurations.some(c =>
      c.type && (String(c.type).toLowerCase().includes("carré") || String(c.label||"").toLowerCase().includes("t-carré")) && c.planets &&
      (c.planets.includes("Soleil") || c.planets.includes("Lune") || c.planets.includes("Ascendant"))
    ));
    ...
    // [4] RICHESSE_PROMISE : Jupiter conjoint Part de Fortune ≤ 3°
    _npAddTag("RICHESSE_PROMISE", _npHasNatalAsp("Jupiter", "Part de Fortune", ["Conjonction"], 3) !== null);
