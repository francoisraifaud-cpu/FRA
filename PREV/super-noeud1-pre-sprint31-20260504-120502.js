...
// (Jupiter|Vénus matched ET impliquant MC/Milieu du Ciel/Soleil dans le détail).
// Le check bénéfique est donc différencié : civil (MARIAGE/ENFANT) = Jupiter/Vénus n'importe où,
// pro (CARRIERE_UP) = Jupiter/Vénus mais ciblant MC ou Soleil (sinon "Jupiter en M2" déclencherait à tort).
...
const _a4ShouldSkipMaleficPenalty = (sig) => {
  ...
  const benM = (sig.matchedConditions || []).find(m => {
    if ((m.strength || 0) < 0.7) return false;
    const detail = String(m.detail || "");
    if (!/Jupiter|Vénus/i.test(detail)) return false;
    if (isPro) {
      // Sprint 3 — pro : doit aussi impliquer MC, Milieu du Ciel ou Soleil
      return /Milieu du Ciel|MC\b|Soleil/i.test(detail);
    }
    return true;
  });
  if (benM) return { rawRatio: sig._rawRatio, reason: isPro ? "benefic_pro_matched" : "benefic_matched", trigger: String(benM.detail || "").slice(0, 60) };
