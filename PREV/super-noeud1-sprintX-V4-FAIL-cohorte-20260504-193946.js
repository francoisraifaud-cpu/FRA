...
if (finalOutput.length > 0) {
    ...
    // --- Sprint X (2026-05-04, validé astrologue retour n°12) : COMPOUNDS DEUIL "convergence extrême" ---
    // Architecture : ne pas toucher aux 25 conditions DEUIL existantes (weights 3-12).
    // Ajouter par-dessus 3 compounds qui captent les patterns "convergence triple" identifiés
    // par l'astrologue : Hyleg attack (Saturne/Pluton conj/carré/opp partile ≤1° sur Soleil/Lune/Asc)
    // + Profection M8/M12 + (Éclipse ≤2° sur point sensible OU transit lourd M8/M12).
    // Si critical_trauma triple convergence : BOOST FORCÉ conf >= 88 (override de la réalité).
    // Cible Marie-Antoinette 1793 (DEUIL 38 → 88+), Lennon 1980 (58 → 85+), Kahlo 1925 ACCIDENT (47 → 85+).
    try {
      // [DEBUG] sentinel "block entered" — visible dans le NDJSON via _sprintXBlockEntered
      _mdseResults.forEach(s => { if (s.code === "DEUIL") s._sprintXBlockEntered = true; });
      // Helper 1 : aspect dur Saturne/Pluton sur Soleil/Lune/Asc avec orbe ≤ orbMax (T→Natal)
      const _subxHylegAttack = (orbMax) => {
        const attackers = ["Saturne","Pluton"];
        const targets = ["Soleil","Lune","Ascendant"];
        for (const att of attackers) {
          for (const tgt of targets) {
            const m = _hasSlowAspectTo(att, tgt, "tendu", orbMax);
            if (m) return { attacker: att, target: tgt, aspect: m.aspect, orb: m.orb, mode: "natal" };
          }
        }
        return null;
      };
      // Helper 1bis : aspect dur Saturne/Pluton (transit) sur Soleil/Lune/Asc PROGRESSÉS
      // Sprint X retour n°13 Q3-B (validé astrologue) : étendre Hyleg attack aux T→P
      // avec orbe ≤ 0.5° strict (compense le caractère lent des progressions secondaires —
      // Lune progressée avance de ~1°/mois → fenêtre prédictive d'un mois).
      // Cible : Lennon 1980 (Saturne carré Lune progressée à 0.03°).
      const _subxHylegAttackProgressed = (orbMax) => {
        try {
          if (!progClosestEntry?.planetes) return null;
          if (!Array.isArray(daysArray) || daysArray.length === 0) return null;
          const attackers = ["Saturne", "Pluton"];
          const progTargets = ["Soleil", "Lune", "Ascendant"];
          const ASPECTS = [{ name: "Conjonction", deg: 0 }, { name: "Carré", deg: 90 }, { name: "Opposition", deg: 180 }];
          let best = null;
          for (const att of attackers) {
            for (const tgt of progTargets) {
              const tgtData = progClosestEntry.planetes[tgt];
              const progDeg = tgtData?.fullDegree;
              if (progDeg == null) continue;
              for (const day of daysArray) {
                const tDeg = day.planetes?.[att]?.fullDegree;
                if (tDeg == null) continue;
                let diff = Math.abs(tDeg - progDeg); if (diff > 180) diff = 360 - diff;
                for (const a of ASPECTS) {
                  const orb = Math.abs(diff - a.deg);
                  if (orb <= orbMax && (!best || orb < best.orb)) {
                    best = { attacker: att, target: tgt, mode: "progressed", aspect: a.name, orb: Math.round(orb * 100) / 100, date: day.date };
                  }
                }
              }
            }
      ...
      // Helper 5 : Mars (rapide) ou Uranus (lent) en aspect dur partile (≤ orbMax)
      // avec planète natale en M8. Sprint X retour n°13 Q1-D (validé astrologue) :
      // compound parallèle compound_sudden_end = Hyleg + Mars/Uranus partile M8 + éclipse récente.
      // Cible : drames violents et soudains (guillotine, assassinat) qui échappent
      // parfois à la triple convergence canonique (Marie-Antoinette 1793 profection M2).
      const _subxMarsUranusPartileOnM8Natal = (orbMax) => {
        try {
          const targetsM8 = Object.entries(natalDict)
            .filter(([n, nd]) => nd?.house === 8 && !["Ascendant","Descendant","Milieu du Ciel","Imum Coeli"].includes(n))
            .map(([n]) => n);
          if (targetsM8.length === 0) return null;
          const HARD = ["Conjonction", "Carré", "Opposition"];
          for (const tgt of targetsM8) {
            for (const aspName of HARD) {
              const k = `fast_Mars|||${aspName}|||${tgt}`;
              if (typeof aspectsSuiviRap !== "undefined" && aspectsSuiviRap?.[k]) {
                ...
                if (minOrb != null && minOrb <= orbMax)
                  return { attacker: "Mars", target: tgt, aspect: aspName, orb: Math.round(minOrb * 100) / 100 };
              }
            }
          }
          for (const tgt of targetsM8) {
            const u = (typeof _hasSlowAspectTo === "function") ? _hasSlowAspectTo("Uranus", tgt, "tendu", orbMax) : null;
            if (u) return { attacker: "Uranus", target: u.target, aspect: u.aspect, orb: u.orb };
          }
        } catch (_) {}
        return null;
      };
      // Helper 2 : éclipse à orbe ≤ orbMax d'un point natal sensible
      // Sprint X retour n°14 Q2-B (validé astrologue) : étendre la fenêtre temporelle aux
      // éclipses des 12 mois précédant la période (orbe spatial reste strict ≤ 2° sur Hyleg).
      // Justification Ptolémée : une éclipse ensemence un point du zodiaque ; un transit lourd
      // qui frappe ce point 6-9 mois plus tard agit en chambre d'écho de l'éclipse précédente.
      const _subxEclipseOnPoint = (targetNames, orbMax, lookbackMonths) => {
        if (!Array.isArray(eclipsesData) || eclipsesData.length === 0) return null;
        if (!Array.isArray(daysArray) || daysArray.length === 0) return null;
        const _LB = lookbackMonths || 12;
        const day0 = daysArray[0];
        const sun0 = day0?.planetes?.Soleil?.fullDegree;
        const day0Date = day0?.date ? new Date(day0.date) : null;
        const SUN_DEG_PER_DAY = 0.98561; // vitesse moyenne Soleil (équinoxe à équinoxe)
        for (const e of eclipsesData) {
          let refDeg = null;
          let inWindow = false;
          const dayMatch = daysArray.find(d => d.date === e.date);
          if (dayMatch?.planetes) {
            refDeg = e.astre === "Soleil" ? dayMatch.planetes.Soleil?.fullDegree : dayMatch.planetes.Lune?.fullDegree;
            inWindow = true;
          } else if (sun0 != null && day0Date) {
            // Éclipse antérieure à la période — extrapoler depuis day0 si dans la fenêtre lookback
            const eDate = new Date(e.date);
            if (!isNaN(eDate)) {
              const daysBack = (day0Date - eDate) / 86400000;
              if (daysBack > 0 && daysBack <= _LB * 30) {
                const eclSunDeg = ((sun0 - daysBack * SUN_DEG_PER_DAY) % 360 + 360) % 360;
                // Pour une éclipse de Lune, la Lune est en opposition Soleil (axe nodal éclipsé)
                refDeg = e.astre === "Soleil" ? eclSunDeg : ((eclSunDeg + 180) % 360);
                inWindow = true;
              }
            }
          }
          if (refDeg == null || !inWindow) continue;
          for (const tgt of targetNames) {
            const nd = natalDict[tgt];
            if (!nd?.deg && nd?.deg !== 0) continue;
            let d = Math.abs(refDeg - nd.deg); if (d > 180) d = 360 - d;
            if (d <= orbMax) return {
              date: e.date, astre: e.astre, target: tgt,
              orb: Math.round(d * 10) / 10,
              lookback: !dayMatch
            };
          }
        }
      ...
      // Helper 3 : transit lourd dans M8 ou M12
      const _subxTransitInM8M12 = () => {
        const planets = ["Mars","Saturne","Uranus","Pluton"];
        for (const p of planets) {
          for (const h of [8, 12]) {
            if (_slowPlanetTraversesHouse(p, h)) return { planet: p, house: h };
          }
        }
        return null;
      };
      // Helper 5 (V4 retour n15 valide astrologue) : Mars/Uranus en aspect dur partile a un point natal donne.
      // Sert au filtre "violence" de LORD_ATTACKS_HYLEG : sans intervention de la "lame" (Mars/Uranus),
      // une attaque saturnienne reste epreuve symbolique (fardeau, prise de pouvoir) plutot que mort physique.
      const _subxMarsUranusPartileOnPoint = (target, orbMax) => {
        try {
          const HARD = ["Conjonction", "Carre", "Opposition"];
          for (const aspName of HARD) {
            const k = "fast_Mars|||" + aspName + "|||" + target;
            if (typeof aspectsSuiviRap !== "undefined" && aspectsSuiviRap?.[k]) {
              const orbs = (aspectsSuiviRap[k].waves || []).map(w => parseFloat(w.minOrb)).filter(x => isFinite(x));
              const minOrb = orbs.length ? Math.min(...orbs) : null;
              if (minOrb != null && minOrb <= orbMax)
                return { attacker: "Mars", target, aspect: aspName, orb: Math.round(minOrb * 100) / 100 };
            }
          }
          const u = (typeof _hasSlowAspectTo === "function") ? _hasSlowAspectTo("Uranus", target, "tendu", orbMax) : null;
          if (u) return { attacker: "Uranus", target: u.target, aspect: u.aspect, orb: u.orb };
        } catch(_) {}
        return null;
      };
      // Helper 6 (V4 retour n15 valide astrologue) : Jupiter/Venus en aspect HARMONIQUE partile sur Hyleg.
      // Veto benefique (Ptolemee, Dorotheus) : un transit fatal ne peut pas tuer si le Soleil/Lune/Asc
      // est protege par Jupiter ou Venus en conjonction, trigone ou sextile partile.
      // Cible : retirer Chirac 1995 / Obama 2008 / Mitterrand 1981 (Jupiter/Venus protege l'Hyleg
      // pendant la prise de pouvoir presidentielle).
      const _subxJupiterVenusProtects = (target, orbMax) => {
        try {
          const benefics = ["Jupiter", "Venus"];
          const HARMONIC = ["Conjonction", "Trigone", "Sextile"];
          for (const ben of benefics) {
            // Aspects rapides : Mars/Soleil/Mercure/Venus/Lune passent vite ; Jupiter/Saturne/etc. sont lents
            // Venus est rapide ; Jupiter est lent.
            for (const aspName of HARMONIC) {
              const kFast = "fast_" + ben + "|||" + aspName + "|||" + target;
              if (typeof aspectsSuiviRap !== "undefined" && aspectsSuiviRap?.[kFast]) {
                const orbs = (aspectsSuiviRap[kFast].waves || []).map(w => parseFloat(w.minOrb)).filter(x => isFinite(x));
                const minOrb = orbs.length ? Math.min(...orbs) : null;
                if (minOrb != null && minOrb <= orbMax)
                  return { protector: ben, target, aspect: aspName, orb: Math.round(minOrb * 100) / 100, source: "fast" };
              }
            }
            // Aspects lents (Jupiter notamment) - on accepte Conjonction + Trigone + Sextile partile.
            // _hasSlowAspectTo mode "harmonique" = Trigone+Sextile uniquement -> on cherche directement
            // dans aspectsSuiviLent pour aussi capter la Conjonction Jupiter au Soleil natal (tres protectrice).
            for (const aspName of HARMONIC) {
              const kSlow = "slow_" + ben + "|||" + aspName + "|||" + target;
              if (typeof aspectsSuiviLent !== "undefined" && aspectsSuiviLent?.[kSlow]) {
                const orbs = (aspectsSuiviLent[kSlow].waves || []).map(w => parseFloat(w.minOrb)).filter(x => isFinite(x));
                const minOrb = orbs.length ? Math.min(...orbs) : null;
                if (minOrb != null && minOrb <= orbMax)
                  return { protector: ben, target, aspect: aspName, orb: Math.round(minOrb * 100) / 100, source: "slow" };
              }
            }
          }
        } catch(_) {}
      ...
      // Helper 4 : profection annuelle M8/M12 OU maître M8/M12 activé
      // Sprint X retour n°13 Q1-C (validé astrologue) : élargir au maître planétaire.
      // En astrologie hellénistique/médiévale, une maison est activée soit par profection
      // directe, soit par activation de son maître. Cible : Marie-Antoinette 1793 (profection M2
      // mais maître de M8 potentiellement activé).
      const _subxProfectionM8M12 = () => {
        const ph = profections?.annuelle?.maison;
        if (ph === 8 || ph === 12) return { house: ph, mode: "direct" };
        try {
          const maitreM8  = (typeof getMaitreProfection === "function" && typeof getProfectionSign === "function")
            ? getMaitreProfection(getProfectionSign(8))  : null;
          const maitreM12 = (typeof getMaitreProfection === "function" && typeof getProfectionSign === "function")
            ? getMaitreProfection(getProfectionSign(12)) : null;
          const seigneurAn = profections?.annuelle?.seigneur;
          if (seigneurAn && maitreM8  && seigneurAn === maitreM8)
            return { house: 8,  mode: "lord_is_seigneur",  lord: maitreM8 };
          if (seigneurAn && maitreM12 && seigneurAn === maitreM12)
            return { house: 12, mode: "lord_is_seigneur",  lord: maitreM12 };
          // Sprint X V4 retour n15 (valide astrologue, Proposition C-2) :
          // retrait du mode "lord_in_profected" (precision 9.5% cohorte = 19 FP / 21 activations).
          // Garde uniquement (a) profection M8/M12 directe et (b) seigneurAn === maitre M8/M12.
        } catch(_) {}
        return null;
      };
      ...
      _mdseResults.forEach(sig => {
        ...
        sig._sprintXReached = true; // [DEBUG]
        // Hyleg attack — natal (T→Natal) prioritaire, puis progressé (T→P) en filet de sécurité (Q3-B)
        const hylegNatal = _subxHylegAttack(1.0);
        const hylegProg  = hylegNatal ? null : _subxHylegAttackProgressed(0.5);
        const hyleg      = hylegNatal || hylegProg;
        const profCrit    = _subxProfectionM8M12();
        const eclipseHit  = _subxEclipseOnPoint(["Soleil","Lune","Ascendant"], 2.0);
        const transitCrit = _subxTransitInM8M12();
        const marsUranusM8 = _subxMarsUranusPartileOnM8Natal(1.0); // Q1-D
        const compounds = [];
        // C1 : Hyleg attack (natal weight 22, progressé weight 18)
        if (hyleg) {
          const isProg = hyleg.mode === "progressed";
          compounds.push({
            type: isProg ? "compound_hyleg_attack_progressed" : "compound_hyleg_attack",
            weight: isProg ? 18 : 22,
            strength: 1.0,
            detail: isProg
              ? `Sprint X — Attaque Hyleg (T→Progressé) : ${hyleg.attacker} ${hyleg.aspect} ${hyleg.target} progressé (orbe ${hyleg.orb}°, ${hyleg.date})`
              : `Sprint X — Attaque Hyleg : ${hyleg.attacker} ${hyleg.aspect} ${hyleg.target} natal (orbe ${hyleg.orb}°)`
          });
        }
        // C2 : Profection M8/M12 OU maître M8/M12 activé (Q1-C)
        if (profCrit) {
          const profDetail = profCrit.mode === "direct"
            ? `Profection annuelle directe sur M${profCrit.house}`
            : profCrit.mode === "lord_is_seigneur"
              ? `Maître de M${profCrit.house} (${profCrit.lord}) = seigneur de l'année`
              : `Maître de M${profCrit.house} (${profCrit.lord}) résidant natalement dans la maison profectée`;
          compounds.push({
            type: "compound_profection_critical",
            weight: 12,
            strength: 1.0,
            detail: `Sprint X — ${profDetail} (climat à risque)`
          });
        }
        // C3 : Éclipse sur Hyleg
        if (eclipseHit) {
          compounds.push({
            type: "compound_eclipse_on_hyleg",
            weight: 18,
            strength: 1.0,
            detail: `Sprint X — Éclipse ${eclipseHit.astre} (${eclipseHit.date}) ≤2° de ${eclipseHit.target} natal (orbe ${eclipseHit.orb}°)`
          });
        }
        // C4 : Transit M8/M12
        if (transitCrit) {
          compounds.push({
            type: "compound_transit_M8M12",
            weight: 12,
            strength: 1.0,
            detail: `Sprint X — Transit lourd ${transitCrit.planet} en M${transitCrit.house} natale`
          });
        }
        // C5 : Mars/Uranus partile M8 (Q1-D)
        if (marsUranusM8) {
          compounds.push({
            type: "compound_mars_uranus_M8_partile",
            weight: 12,
            strength: 1.0,
            detail: `Sprint X — ${marsUranusM8.attacker} ${marsUranusM8.aspect} ${marsUranusM8.target} (M8 natal) à orbe ${marsUranusM8.orb}° (fin brutale)`
          });
        }
        // CRITICAL_TRAUMA = Hyleg (natal OU progressé) + Profection (directe OU maître) + (Éclipse OU Transit)
        const criticalTrauma = !!(hyleg && profCrit && (eclipseHit || transitCrit));
        if (criticalTrauma) {
          compounds.push({
            type: "compound_critical_trauma",
            weight: 30,
            strength: 1.0,
            detail: `Sprint X — CRITICAL_TRAUMA : Hyleg(${hyleg.attacker}→${hyleg.target}${hyleg.mode === "progressed" ? " prog" : ""}) + Prof${profCrit.mode === "direct" ? `M${profCrit.house}` : `M${profCrit.house}via${profCrit.lord}`} + ${eclipseHit ? `Éclipse(${eclipseHit.target})` : `TransitM${transitCrit.house}(${transitCrit.planet})`}`
          });
        }
        // SUDDEN_END desactive en V4 (retour n15) : 0/1 precision sur cohorte n=100
        // (Reagan promotion 1981 = seul declenchement, FP). Helper marsUranusM8 garde pour debug.
        const suddenEnd = false;
        // LORD_ATTACKS_HYLEG V4 (valide astrologue retour n15, B+C) :
        // baseCondition : Hyleg natal partile + attacker EST maitre M8/M12 + attacker natal en M8/M12
        // hasViolence    : Mars OU Uranus en aspect dur partile au lord OU au target Hyleg (la "lame")
        // hasProtection  : Jupiter OU Venus en aspect harmonique partile au target Hyleg (veto benefique Ptolemee)
        // RESULTAT :
        //   - baseCondition && hasViolence && !hasProtection -> BOOST DEUIL 88 + compound mort fatale
        //   - baseCondition && hasProtection -> tag "FARDEAU_INSTITUTIONNEL" sur CARRIERE_UP, PAS de boost DEUIL
        //   - baseCondition seul (sans violence ni protection) -> rien (epreuve transformatrice non documentee)
        let lordAttackInfo = null;
        let lordBaseCondition = false;
        let lordHasViolence = null;
        let lordHasProtection = null;
        if (hyleg && hyleg.mode === "natal") {
          try {
            ...
            const _maitreM12 = (typeof getMaitreProfection === "function" && typeof getProfectionSign === "function")
              ? getMaitreProfection(getProfectionSign(12)) : null;
            const attacker = hyleg.attacker;
            const isLordM8  = !!(_maitreM8  && attacker === _maitreM8);
            const isLordM12 = !!(_maitreM12 && attacker === _maitreM12);
            if (isLordM8 || isLordM12) {
              const lordHouse = natalDict[attacker]?.house;
              if (lordHouse === 8 || lordHouse === 12) {
                lordBaseCondition = true;
                lordAttackInfo = {
                  attacker, target: hyleg.target, aspect: hyleg.aspect, orb: hyleg.orb,
                  lordOf: isLordM8 ? 8 : 12,
                  natalHouse: lordHouse
                };
                // hasViolence : Mars/Uranus partile sur attacker OU sur target Hyleg (orbe <= 1.5)
                const violOnAttacker = _subxMarsUranusPartileOnPoint(attacker, 1.5);
                const violOnTarget   = _subxMarsUranusPartileOnPoint(hyleg.target, 1.5);
                lordHasViolence = violOnAttacker || violOnTarget;
                // hasProtection : Jupiter/Venus aspect harmonique partile sur target Hyleg (orbe <= 2.0)
                lordHasProtection = _subxJupiterVenusProtects(hyleg.target, 2.0);
              }
            }
          } catch(_) {}
        }
        // boolean final pour le boost
        const lordAttacksHyleg = lordBaseCondition && lordHasViolence && !lordHasProtection;
        // CARRIERE_UP tagging (cas fardeau institutionnel) : si baseCondition + protection -> tag, pas de boost DEUIL
        const lordIsFardeau = lordBaseCondition && !!lordHasProtection;
        if (lordAttacksHyleg) {
          compounds.push({
            type: "compound_lord_attacks_hyleg",
            weight: 30,
            strength: 1.0,
            detail: "Sprint X V4 LORD_ATTACKS_HYLEG : " + lordAttackInfo.attacker + " (maitre M" + lordAttackInfo.lordOf + " en M" + lordAttackInfo.natalHouse + " natal) " + lordAttackInfo.aspect + " " + lordAttackInfo.target + " partile orbe " + lordAttackInfo.orb + " + violence " + lordHasViolence.attacker + " partile " + lordHasViolence.target + " (le faucheur attitre frappe l'Hyleg avec la lame)."
          });
        }
        // [DEBUG] diagnostic profection détaillé (pour comprendre pourquoi Q1-C ne match pas)
        let _profDbg = null;
        try {
          const _maitreM8  = (typeof getMaitreProfection === "function" && typeof getProfectionSign === "function")
            ? getMaitreProfection(getProfectionSign(8))  : null;
          const _maitreM12 = (typeof getMaitreProfection === "function" && typeof getProfectionSign === "function")
            ? getMaitreProfection(getProfectionSign(12)) : null;
          _profDbg = {
            maisonProfectee: profections?.annuelle?.maison || null,
            seigneurAn: profections?.annuelle?.seigneur || null,
            maitreM8: _maitreM8, maitreM12: _maitreM12,
            houseM8Lord: _maitreM8 ? (natalDict[_maitreM8]?.house || null) : null,
            houseM12Lord: _maitreM12 ? (natalDict[_maitreM12]?.house || null) : null
          };
        } catch(_) {}
        sig._sprintXCompounds = {
          hylegAttack: hyleg,
          hylegMode: hyleg ? hyleg.mode : null,
          profectionCritical: profCrit,
          profectionDebug: _profDbg,
          eclipseOnHyleg: eclipseHit,
          transitM8M12: transitCrit,
          marsUranusM8Partile: marsUranusM8,
          lordAttackInfo,
          lordBaseCondition,
          lordHasViolence,
          lordHasProtection,
          criticalTrauma,
          suddenEnd,
          lordAttacks: lordAttacksHyleg,
          lordIsFardeau,
          compoundsCount: compounds.length
        };
        if (compounds.length > 0) {
          sig.matchedConditions = (sig.matchedConditions || []).concat(compounds);
        }
        // BOOST FORCE : critical_trauma OU lord_attacks_hyleg (avec violence + sans protection)
        // V4 (retour n15) : sudden_end desactive (0% precision), lord_attacks resserre par
        // triple condition (base + violence + !protection).
        if ((criticalTrauma || lordAttacksHyleg) && sig.confidence < 88) {
          sig._criticalTraumaBoost = {
            before: sig.confidence,
            after: 88,
            reason: criticalTrauma
              ? "Triple convergence Hyleg + Profection (M8/M12 directe ou seigneurAn=maitre) + (Eclipse OU Transit lourd M8/M12)"
              : "LORD_ATTACKS_HYLEG V4 : " + lordAttackInfo.attacker + " (maitre M" + lordAttackInfo.lordOf + " en M" + lordAttackInfo.natalHouse + ") " + lordAttackInfo.aspect + " " + lordAttackInfo.target + " orbe " + lordAttackInfo.orb + " + violence " + lordHasViolence.attacker + " partile + pas de protection Jupiter/Venus"
          };
          sig.confidence = 88;
          sig.level = "high";
        }
      });
      // V4 (retour n15 C) : tagging CARRIERE_UP "FARDEAU_INSTITUTIONNEL" si lordIsFardeau
      // (baseCondition LORD_ATTACKS_HYLEG + protection Jupiter/Venus). Pas de boost DEUIL.
      // Cible : Chirac 1995 / Obama 2008 / Mitterrand 1981 — preservation top-1 CARRIERE_UP.
      try {
        const fardeauDeuil = _mdseResults.find(s => s.code === "DEUIL" && s._sprintXCompounds?.lordIsFardeau);
        if (fardeauDeuil) {
          const protInfo = fardeauDeuil._sprintXCompounds?.lordHasProtection;
          const lordInfo = fardeauDeuil._sprintXCompounds?.lordAttackInfo;
          _mdseResults.forEach(carr => {
            if (carr.code !== "CARRIERE_UP") return;
            carr.qualifierTags = Array.isArray(carr.qualifierTags) ? carr.qualifierTags : [];
            if (!carr.qualifierTags.includes("FARDEAU_INSTITUTIONNEL")) {
              carr.qualifierTags.push("FARDEAU_INSTITUTIONNEL");
            }
            carr._fardeauInstitutionnel = {
              source: "Sprint X V4 LORD_ATTACKS_HYLEG + protection Jupiter/Venus",
              attacker: lordInfo?.attacker,
              lordOf: lordInfo?.lordOf,
              target: lordInfo?.target,
              orb: lordInfo?.orb,
              protector: protInfo?.protector,
              protectorAspect: protInfo?.aspect,
              protectorOrb: protInfo?.orb,
              label: "Fardeau institutionnel / epreuve transformatrice (saturne maitre 8/12 attaque Hyleg, vie protege par Jupiter/Venus)"
            };
          });
        }
      } catch(_) {}
    } catch(e) {
      console.log("[WARN] Sprint X DEUIL compounds:", e.message);
      try { _mdseResults.forEach(s => { if (s.code === "DEUIL") s._sprintXErr = String(e.message || e).slice(0, 300); }); } catch(_) {}
    }
    ...
    try {
      ...
      // Sprint X retour n°13 (validé astrologue, question bonus) : seuil "drame écrase l'année"
      // Quand le compétiteur dépasse 85, le test peakWindow overlap est ignoré (un événement
      // comme la mort écrase biographiquement toute l'année). Cible : Chopin/Grace/Lennon
      // qui passaient à travers le filtre Sprint 3.4 par peakWindow disjoint.
      const _CARRIERE_COMPETITOR_OVERRIDE_THRESHOLD = 85;
      ...
      _mdseResults.forEach(sig => {
        ...
        const conflict = competitors.find(c =>
          (c.confidence || 0) >= _CARRIERE_COMPETITOR_OVERRIDE_THRESHOLD || _windowsOverlap(sig.peakWindow, c.peakWindow)
        );
        if (conflict) {
          const tagDef = _CARRIERE_TAG_BY_COMPETITOR[conflict.code] || { tag: "CARRIERE_UP_OMBRE", label: "Carrière sous tension" };
          const isYearOverride = (conflict.confidence || 0) >= _CARRIERE_COMPETITOR_OVERRIDE_THRESHOLD && !_windowsOverlap(sig.peakWindow, conflict.peakWindow);
          sig._carriereDemoted = {
            ...
            tag: tagDef.tag,
            label: tagDef.label,
            reason: isYearOverride
              ? `Filtre concurrence (drame écrase l'année) : ${conflict.code} conf=${conflict.confidence} >= ${_CARRIERE_COMPETITOR_OVERRIDE_THRESHOLD} (peakWindow disjoint mais score absolu critique)`
              : `Filtre concurrence : ${conflict.code} conf=${conflict.confidence} >= ${_CARRIERE_COMPETITOR_THRESHOLD} overlap fenetre`
          };
    ...
    if (finalOutput.length > 0) {
        finalOutput[0].json.heatmapData = {
            ...
            eventSignatures: _mdseResults.map(s => ({
                ...
                sprintXCompounds:s._sprintXCompounds||null,
                sprintXBlockEntered:s._sprintXBlockEntered||null,
                sprintXReached:s._sprintXReached||null,
                sprintXErr:s._sprintXErr||null,
                criticalTraumaBoost:s._criticalTraumaBoost||null,
