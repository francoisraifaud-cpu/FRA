...
const _natalPromises = (() => {
  const out = {
    ...
    qualitativeTags: [],
    stellarTags: [],
    bySignature: { SANTE_DOWN: [], DEUIL: [], ACCIDENT: [], SEPARATION: [], MARIAGE: [], FINANCE_DOWN: [], CARRIERE_UP: [] }
  };
  try {
...
try {
  const _STAR_CODE_MAP = {
    ...
    "Spica":     { codes:["FINANCE_UP","JURIDIQUE_UP","SANTE_UP","MARIAGE"], boost:0.10 },
    ...
    "Mirfak":    { codes:["CARRIERE_UP","JURIDIQUE_UP"], boost:0.08 },
  ...
  for (const em of (etoileMatchesNatal || [])) {
    ...
    info.codes.forEach(code => {
      ...
      _mdseFixedStarHits[code].push({ star: em.etoile, planet: em.planete, orb: Math.round(orb*100)/100, boost: info.boost });
    });
  }
  for (const ec of (etoileCuspMatchesNatal || [])) {
    ...
    info.codes.forEach(code => {
      if (!_mdseFixedStarHits[code]) _mdseFixedStarHits[code] = [];
      _mdseFixedStarHits[code].push({ star: ec.etoile, planet: ec.angle, orb: Math.round(orb*100)/100, boost: info.boost, isAngle: true });
    });
  }
...
if (finalOutput.length > 0) {
    ...
    try {
        ...
        const _A44_CAP      = 95;
        const _A44_ANGLE_MULT = { "Ascendant": 1.0, "Milieu du Ciel": 1.0, "Descendant": 0.6, "Imum Coeli": 0.6, "Fond du Ciel": 0.6 };
        ...
        for (const h of _a44Final) {
            ...
            // Garde-fou anti-régression M1 : si la polarité est maléfique ET le top-1
            // pré-boost est constructif, plafonner le boost à (top1Before.confidence - 1).
            // Sinon ratchet déclencherait (cf. cas Rowling 1997 : Saturne Carré MC poussait
            // JURIDIQUE_DOWN 85→89 au-dessus de CARRIERE_UP=85 top-1 → régression).
