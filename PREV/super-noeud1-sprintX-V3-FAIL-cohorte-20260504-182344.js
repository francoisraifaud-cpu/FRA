    ...
    try {
      ...
      const _subxProfectionM8M12 = () => {
        ...
        try {
          ...
          const houseM8Lord  = maitreM8  ? natalDict[maitreM8]?.house  : null;
          const houseM12Lord = maitreM12 ? natalDict[maitreM12]?.house : null;
          if (ph != null && houseM8Lord  != null && ph === houseM8Lord)
            return { house: 8,  mode: "lord_in_profected", lord: maitreM8 };
          if (ph != null && houseM12Lord != null && ph === houseM12Lord)
            return { house: 12, mode: "lord_in_profected", lord: maitreM12 };
        } catch(_) {}
        return null;
      };
      ...
      _mdseResults.forEach(sig => {
        ...
        // SUDDEN_END = Hyleg + Mars/Uranus partile M8 + Éclipse récente sur Hyleg (Q1-D)
        // Compound parallèle pour les fins brutales/violentes (guillotine, assassinat)
        // qui n'ont pas la triple convergence canonique mais ont le pattern "violence + M8 + éclipse".
        const suddenEnd = !!(hyleg && marsUranusM8 && eclipseHit);
        if (suddenEnd) {
          compounds.push({
            type: "compound_sudden_end",
            weight: 28,
            strength: 1.0,
            detail: `Sprint X — SUDDEN_END : Hyleg(${hyleg.attacker}→${hyleg.target}) + ${marsUranusM8.attacker} partile M8 (${marsUranusM8.target}) + Éclipse récente sur ${eclipseHit.target} (fin brutale)`
          });
        }
        // LORD_ATTACKS_HYLEG = Hyleg natal partile + attacker EST le maître M8/M12 + attacker natal en M8/M12
        // Sprint X retour n°14 Q1 (validé astrologue, Proposition A) : "le faucheur attitré du thème
        // sort de sa propre maison de la mort pour frapper l'Hyleg à bout portant". Cible Marie-Antoinette
        // 1793 : Saturne (maître M8 résident en M8) opp Soleil natal partile orbe 0°.
        // Contrainte topologique très forte : maître M8/M12 natalement en M8/M12 → ~1/6 des thèmes
        // ET aspect dur partile sur Hyleg ce sprint → activation extrêmement rare.
        let lordAttacksHyleg = false;
        ...
        if (lordAttacksHyleg) {
          compounds.push({
            ...
            weight: 30,
            strength: 1.0,
            detail: `Sprint X — LORD_ATTACKS_HYLEG : ${lordAttackInfo.attacker} (maître M${lordAttackInfo.lordOf} natalement en M${lordAttackInfo.natalHouse}) ${lordAttackInfo.aspect} ${lordAttackInfo.target} natal partile orbe ${lordAttackInfo.orb}° — le faucheur attitré frappe l'Hyleg à bout portant.`
          });
        }
        ...
        sig._sprintXCompounds = {
          ...
          lordAttacksHyleg: lordAttackInfo,
          criticalTrauma,
          suddenEnd,
        ...
        // BOOST FORCÉ : si critical_trauma OU sudden_end OU lord_attacks_hyleg → conf >= 88
        // Q2-A (validé astrologue retour n°13) : pas de boost sur Hyleg seul (cas Mitterrand 1981).
        // Q1 retour n°14 (validé astrologue) : ajout lord_attacks_hyleg comme 3e voie de boost.
        if ((criticalTrauma || suddenEnd || lordAttacksHyleg) && sig.confidence < 88) {
          sig._criticalTraumaBoost = {
            before: sig.confidence,
            after: 88,
            reason: criticalTrauma
              ? "Triple convergence Hyleg + Profection (M8/M12 ou maître) + (Éclipse OU Transit lourd M8/M12)"
              : suddenEnd
                ? "Sudden end : Hyleg + Mars/Uranus partile M8 + Éclipse récente"
                : `Lord of death attacks Hyleg : ${lordAttackInfo.attacker} (maître M${lordAttackInfo.lordOf} en M${lordAttackInfo.natalHouse} natal) ${lordAttackInfo.aspect} ${lordAttackInfo.target} natal partile orbe ${lordAttackInfo.orb}°`
          };
          sig.confidence = 88;
          sig.level = "high";
        }
