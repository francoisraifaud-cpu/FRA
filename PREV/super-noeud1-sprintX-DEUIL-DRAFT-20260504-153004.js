...
if (finalOutput.length > 0) {
    ...
    try {
      // Helper 1 : aspect dur Saturne/Pluton sur Soleil/Lune/Asc avec orbe ≤ orbMax
      const _subxHylegAttack = (orbMax) => {
        ...
        for (const att of attackers) {
          for (const tgt of targets) {
            ...
            if (m) return { attacker: att, target: tgt, aspect: m.aspect, orb: m.orb };
          }
        }
        return null;
      };
      ...
      // Helper 4 : profection annuelle M8 ou M12
      const _subxProfectionM8M12 = () => {
        const ph = profections?.annuelle?.maison;
        if (ph === 8 || ph === 12) return { house: ph };
        return null;
      };

      _mdseResults.forEach(sig => {
        if (sig.code !== "DEUIL") return;
        const hyleg = _subxHylegAttack(1.0);
        ...
        const compounds = [];
        // C1 : Hyleg attack (compound seul, weight 22 ajouté)
        if (hyleg) {
          compounds.push({
            type: "compound_hyleg_attack",
            weight: 22,
            strength: 1.0,
            detail: `Sprint X — Attaque Hyleg : ${hyleg.attacker} ${hyleg.aspect} ${hyleg.target} natal (orbe ${hyleg.orb}°)`
          });
        }
        // C2 : Profection M8/M12 (compound seul, weight 12 ajouté)
        if (profCrit) {
          compounds.push({
            type: "compound_profection_critical",
            weight: 12,
            strength: 1.0,
            detail: `Sprint X — Profection annuelle M${profCrit.house} (climat à risque)`
          });
        }
        // C3 : Éclipse sur Hyleg (compound seul, weight 18 ajouté)
        if (eclipseHit) {
          compounds.push({
        ...
        // C4 : Transit M8/M12 (compound seul, weight 12 ajouté)
        if (transitCrit) {
          compounds.push({
        ...
        // CRITICAL_TRAUMA = triple convergence (Hyleg + Profection + (Éclipse OU Transit))
        ...
        if (criticalTrauma) {
          compounds.push({
            ...
            weight: 30,
            strength: 1.0,
            detail: `Sprint X — CRITICAL_TRAUMA : Hyleg(${hyleg.attacker}→${hyleg.target}) + ProfM${profCrit.house} + ${eclipseHit ? `Éclipse(${eclipseHit.target})` : `TransitM${transitCrit.house}(${transitCrit.planet})`}`
          });
        }
        // Ajout des compounds matchés à matchedConditions (pour traçabilité audit)
        ...
        // BOOST FORCÉ : si critical_trauma → conf >= 88 (override "convergence absolue")
        if (criticalTrauma && sig.confidence < 88) {
          sig._criticalTraumaBoost = {
            before: sig.confidence,
            after: 88,
            reason: "Triple convergence Hyleg + Profection M8/M12 + (Éclipse OU Transit lourd M8/M12)"
          };
          sig.confidence = 88;
          sig.level = "high";
        }
